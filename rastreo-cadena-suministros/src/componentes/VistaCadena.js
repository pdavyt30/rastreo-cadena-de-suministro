...
const VistaCadena = ({ dificultad }) => {
    ...
    const [etapas, setEtapas] = useState({ abastecimiento: null, produccion: null, almacenamiento: null });
    ...
    const iniciarSimulacion = async () => {
        try {
            const response = await axios.post('http://localhost:8080/api/simulacion/iniciar', etapas);
    ...
    return (
        <div className="container">
            ...
            <div className="button-container">
                <button className="stage-button" onClick={() => handleButtonClick('abastecimiento')} disabled={enEjecucion}>
                    {etapas.abastecimiento ? 'Editar Abastecimiento' : 'Configurar Abastecimiento'}
                </button>
                <button className="stage-button" onClick={() => handleButtonClick('produccion')} disabled={enEjecucion}>
                    {etapas.produccion ? 'Editar Producción' : 'Configurar Producción'}
                </button>
                <button className="stage-button" onClick={() => handleButtonClick('almacenamiento')} disabled={enEjecucion}>
